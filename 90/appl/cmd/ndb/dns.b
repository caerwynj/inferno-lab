...
#
# mdns additions by Eric Van Hensbergen (ericvh@gmail.com)
#
...
init(nil: ref Draw->Context, args: list of string)
{
	...
	if(lists == nil)
		cantload(Arg->PATH);
	arg->init(args);
	arg->setusage("dns [-Drhz] [-f dnsfile] [-s serveraddr] [-x mntpt]");
	svcname := "#sdns";
	serveraddrs : list of string;
	while((c := arg->opt()) != 0)
		case c {
		'D' =>
			debug = 1;
		...
		'm' =>
			usemdns = 1;
		's' =>
			serveraddrs = arg->earg()::serveraddrs;
		'x' =>
			mntpt = arg->earg();
	...
	for( l:=serveraddrs; l != nil; l = tl l)
		spawn udpserver(hd l);
...
RR.text(rr: self ref RR): string
{
	...
	Txt =>
		...
		for(; l != nil; l = tl l)
			s += sys->sprint("\t\t%s\n", hd l);
	Srv =>
		s += sys->sprint("\t%d %d %d %s", ar.pri, ar.weight, ar.port, ar.host);
	AAAA =>
		s += sys->sprint("\t");
		for(count := 0; count < len ar.rdata; count++) {
			s += sys->sprint("%d.", int ar.rdata[count]);
		}
	A =>
		if(len ar.rdata == 4){
			a := ar.rdata;
...
dnslookup(name: string, attr: int): (list of string, string)
{
	...
	Ta =>
		...
		* =>
		...
		if(!islocal(name)&&(srv != nil)){	# try the host's map first
			l := srv->iph2a(name);
			if(l != nil)
...
islocal(label: string): int
{
	mdnsdom := ".local";

	for( end := 0; end < len label; end++)
		if( label[end] == ' ')
			break;
	...
	if(debug)
		sys->print("label: %s postfix: %s\n",label, label[beg:end]);
	if (label[beg:end] == mdnsdom)
		return 1;

	return 0;	
}

mdnsservers(): list of ref NS
{
	slist := ref NS("mdnsmulticast",
		ref RR.A("mdnsmulticast", Ta, Cin, 1<<31, 0,
			array[] of {byte 224, byte 0, byte 0, byte 251})::nil, 0, 1) :: nil;
	return slist;
}

getservers(label: string): list of ref NS
{
...
Step1:
	for(tries:=0; tries<10; tries++){
		...
		if (ismdns)
			...
			slist = getservers(label);
			if(slist == nil)
				return (nil, "no dns servers configured");
		}

		(id, query, err1) := mkquery(attr, Cin, label, ismdns);
		if(err1 != nil){
			sys->fprint(stderr, "dns: %s\n", err1);
			return (nil, err1);
		}
		...
		if(debug)
			printnslist(sys->sprint("ns for %s: ", label), slist);
		...
		for(qset := slist; qset != nil; qset = tl qset){
			...
			(dml, err2) := udpquery(fd, id, query, ns.name, hd ns.addr, ismdns);
			if(dml == nil){
				sys->fprint(stderr, "dns: %s: %s\n", ns.name, err2);
			...
			for(l := dml; l != nil; l = tl l) {
				if(debug)
					sys->print("Processing dml, %d remaining\n", len l);
				dm := hd l;
				...
				if(dm.an != nil){
					...
					if(isresponse(dm, attr)) {
						rrl = lists->concat(dm.an, rrl);
						if((ismdns)&&(dm.ar != nil))
							rrl = lists->concat(dm.ar, rrl);
						continue;
					}
...
putrrl(a: array of byte, o: int, rrl: list of ref RR, labs: ref Labels): int
{
	...
	for(; rrl != nil; rrl = tl rrl){
		...
		Txt =>
			for(l := ar.textdata; l != nil; l = tl l)
				o = puts(a, o, hd l);
		Srv =>
			o = put2(a, 0, ar.pri);
			o = put2(a, 0, ar.weight);
			o = put2(a, o, ar.port);
			o = putdn(a, o, ar.host, labs);
		A or
		Other =>
...
getrrl(nr: int, a: array of byte, o: int): (list of ref RR, int)
{
	...
	for(i := 0; i < nr; i++) {
		...
		Ttxt =>
			...
			rr = ref RR.Txt(name, rtype, rclass, ttl, 0, textdata);
		Tsrv =>
			pri, weight, port: int;
			host: string;
			(pri,  o) = get2(a, o);
			...
			(port, o) = get2(a, o);
			(host, o) = getdn(a, o, 0);
			rr = ref RR.Srv(name, rtype, rclass, ttl, 0, pri, weight, port, host);
		Tcname or
		Tns or
		Tptr =>
			(dname, o) = getdn(a, o, 0);
		...
		Taaaa =>
			...
			rdata[0:] = a[o:o+dlen];
			rr = ref RR.AAAA(name, rtype, rclass, ttl, 0, rdata);
			o += dlen;			
		Ta =>
			rdata := array[dlen] of byte;
			rdata[0:] = a[o:o+dlen];
...
mkquery(qtype: int, qclass: int, name: string, ismdns: int): (int, array of byte, string)
{
	...
	if(ismdns) {
		a[Udprport] = byte (MDNSport>>8);
		a[Udprport+1] = byte MDNSport;		
	} else {
...
udpquery(fd: ref Sys->FD, id: int, query: array of byte, sname: string, addr: ref RR, zc: int): (list of ref DNSmsg, string)
{
	timeout: int;
	dml: list of ref DNSmsg;
	error: string;
	...
	* =>
	...
	spawn timer(timeout, c, pidc);
	tpid := <-pidc;
	for(ntries := 0; ntries < 8;){
		if(dml == nil) {
			if(debug){
...
reader(fd: ref Sys->FD, c: chan of array of byte, pidc: chan of int)
{
	pidc <-= sys->pctl(0, nil);
	for(;;){
		buf := array[9000+Udphdrsize] of byte;
		n := sys->read(fd, buf, len buf);
		if(n > 0){
			if(debug)
				sys->print("rcvd %d\n", n);
...
timer(t: int, c: chan of array of byte, pidc: chan of int)
{
	pidc <-= sys->pctl(0, nil);
	for(;;){
		sys->sleep(t);
...
openlisten(addr: string): ref Sys->FD
{
	(ok, c) := sys->announce(addr);
	if(ok < 0){
		sys->fprint(stderr, "dnsd: can't open port: %r\n");
		return nil;
	}

	if(sys->fprint(c.cfd, "headers") <= 0){
		sys->fprint(stderr, "dnsd: can't set headers: %r\n");
		return nil;
	}

	c.dfd = sys->open(c.dir+"/data", Sys->ORDWR);
	if(c.dfd == nil) {
		sys->fprint(stderr, "dnsd: can't open data file\n");
		return nil;
	}
	return c.dfd;
}

cachelookup(name: string, rtype: int, class: int) :list of ref RR
{
	dom := "_me._inferno._tcp.local";
	localaddr := array [4] of byte;
	localaddr[0] = byte 127;
	localaddr[1] = byte 0;
	localaddr[2] = byte 0;
	localaddr[3] = byte 1;
	...
	host := ref RR.Host(dom, Tptr, Cin, now+60, 0, dom);
	srv := ref RR.Srv(dom, Tsrv, Cin, now+60, 0, 0, 0, 6666, "ericvh-desktop.local");
	txt := ref RR.Txt(dom, Ttxt, Cin, now+60, 0, list of {"foo=bar", "boof=head"});
	addr := ref RR.A(dom, Ta, Cin, now+60, 0, localaddr);

	if (name=="_inferno._tcp.local") {
		sys->print("Woot!\n");
		return list of {host, addr, txt, srv};
	}
	return nil;
}

query(buf: array of byte) :array of byte
{
	qm := ref DNSmsg;
	...
	if(len qm.qd > 0) {
		if(debug) {
			sys->print("==== New Message: %d questations ====\n", len qm.qd);
			sys->print("%s\n===============================\n", qm.text());
		}

		qm.an = nil; # this is probably wrong, but keep it simple for now
		for(l := qm.qd; len l > 0; l = tl l) {
			q := hd l;
			rr := cachelookup(q.name, q.rtype, q.class);
			qm.an = lists->concat(rr, qm.an);
		}
	}

	if(len qm.an > 0) {
		sys->print("Should be returning:\n%s\n",qm.text());
		return qm.pack(0);
	} else
		return nil;
}
...
udpserver(addr: string)
{
	for(;;sys->sleep(10*1000)){
		...
		for(;;){
			...
			answer := query(buf[Udphdrsize:n]);
			if(answer == nil)
				continue;

			# reply (answer probably already array of byte dufus)
			r := array of byte answer;
			if(len answer > len buf - Udphdrsize)
				continue;
			buf[Udphdrsize:] = r;
