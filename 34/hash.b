...
{
	return ref HashTable(array[size] of list of ref HashNode);
}

HashTable.find(h: self ref HashTable, key: string): HashVal
{
	j := fun1(key,len h.a);
...
HashTable.insert(h: self ref HashTable, key: string, val: HashVal)
{
	...
	h.a[j] = ref HashNode(key,val) :: h.a[j];
}

HashTable.delete(h:self ref HashTable, key:string)
{
	j := fun1(key,len h.a);
	dl:list of ref HashNode; dl = nil;
	for(q := h.a[j]; q!=nil; q = tl q){
		if((hd q).key!=key)
			dl = (hd q) :: dl;
	}
...
HashTable.all(h:self ref HashTable): list of ref HashNode
{
	dl:list of ref HashNode; dl = nil;
	for(j:=0; j<len h.a; j++)
		for(q:=h.a[j]; q!=nil; q = tl q)
