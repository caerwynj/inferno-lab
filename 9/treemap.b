...
TRect.distance(r: self TRect, rr: TRect): real
{
	return sqrt((rr.x-r.x)**2 +(rr.y-r.y)**2 +(rr.w-r.w)**2 +(rr.h-r.h)**2);
}

TRect.aspect(r: self TRect): real
{
...
slice(items: array of ref Item, bounds: TRect, orient: int, order: int)
{
	total := sum(items);
	if(0)
...
sliceBest(items: array of ref Item, bounds: TRect)
{
	h:= VERTICAL;
...
square(items: array of ref Item, bounds: TRect)
{
	mergesort(items, array[len items] of ref Item);
	_square(items, bounds);
}

_square(items: array of ref Item, bounds: TRect)
{
	if(len items == 0)	
		return;
	if (len items <= 2){
	...
	# height/width
		...
		sliceBest(items[start:mid+1], TRect(x,y,w,h*b));
		_square(items[mid+1:end], TRect(x,y+h*b,w,h*(1.0-b)));
	}else{
		# width/height
		while (mid<end){
		...
		sliceBest(items[start:mid+1], TRect(x,y,w*b,h));
		_square(items[mid+1:end], TRect(x+w*b,y,w*(1.0-b),h));
	}
}
...
btree(items: array of ref Item, bounds: TRect, vertical: int)
{
	if(len items == 0)
		return;
	if(len items == 1){
...
strip(items: array of ref Item, bounds: TRect)
{
	layoutbox := bounds;
	total := sum(items);
...
layout_strip(items: array of ref Item, box: TRect): int
{
	numItems := 0;
	ar := Infinity;
	prevAR := 0.0;
...
horiz_box(items: array of ref Item, box: TRect): real
{
	total := sum(items);
	height := total / box.w;
	width := 0.0;
	...
	for(i:=0;i<len items;i++){
		...
		items[i].bounds = TRect(x, 0.0, width, height);
		x += width;
	}
	return height;
}
