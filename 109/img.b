...
supported(mtype: int) : int
{
	...
	Img->ImageJpeg or
	Img->ImageGif or
	Img->ImageXXBitmap or
	Img->ImageXInfernoBit or
	Img->ImagePng =>
		return 1;
	}
	return 0;
...
ImageSource.getmim(is: self ref ImageSource) : (int, ref MaskedImage)
{
	...
	if(dbgev)
		event("IMAGE_GETMIM", is.width*is.height);
...

	{
		...
		if(is.bs.eof) {
			if(is.mtype == Img->UnknownType) {
				#u := is.bs.req.url;
				#h := is.bs.hdr;
				#h.setmediatype(u.path, is.bs.data);
				is.mtype = is.bs.mtype;
			}
			case is.mtype {
			Img->ImageJpeg =>
				ans = getjpegmim(is);
			...
			Img->ImageXInfernoBit =>
				ans = getbitmim(is);
			Img->ImagePng =>
				ans = getpngmim(is);
			* =>
				is.err = sys->sprint("unsupported image type %d", is.mtype);
				ret = Mimerror;
				ans = nil;
			}
			if(ans != nil)
...
Decode:
	...
	if(h.fields & INTERLACED) {
		...
		if(dbgev)
			event("IMAGE_GETGIFMIM_INTERLACE_START", 0);
		...
		pic = ipic;
		if(dbgev)
			event("IMAGE_GETGIFMIM_INTERLACE_END", 0);
	}
	if(is.width != is.origw || is.height != is.origh) {
		if (is.width < 0)
			is.width = 0;
	...
	if(h.flags & TRANSP) {
		...
		if(dbgev)
			event("IMAGE_GETGIFMIM_MASK_START", 0);
		...
		mpic = nil;
		if(dbgev)
			event("IMAGE_GETGIFMIM_MASK_END", 0);
	}
	if(dbgev)
		event("IMAGE_GETGIFMIM_REMAP_START", 0);
	pic24 := remap24(pic, h.cmap);
#	remap1(pic, width, height, h.cmap);
	if(dbgev)
		event("IMAGE_GETGIFMIM_REMAP_END", 0);
	bgcolor := -1;
	i := h.bgrnd;
...
getjpegmim(is: ref ImageSource) : ref MaskedImage
{
	...
	if(dbgev)
		event("IMAGE_GETJPGMIM", is.width*is.height);
	getjpeghdr(is);
	h := is.jstate;
...
#		remapgrey(chans[0], width, height);
	...
	if(dbgev)
		event("IMAGE_JPG_REMAP_END", 0);
	im := newimage(is, width, height);
	im.writepixels(im.r, chans[0]);
	if(dbgev)
		event("IMAGE_GETJPGMIM_END", 0);
	return newmi(im);
}
...
jpegbaselinescan(is: ref ImageSource) : array of array of byte
{
	...
	if(dbgev)
		event("IMAGE_JPGBASELINESCAN_END", 0);
	return chans;
}
...
#			sys->print("Finished\n");
	...
	CRGB =>
		...
		r := chanstopix(raw.chans, width, height);
		im := newimage24(is, width, height);
		im.writepixels(im.r, r);
		return newmi(im);
	CRGB1 =>
