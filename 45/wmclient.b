...
window(ctxt: ref Draw->Context, title: string, buts: int): ref Window
{
	...
	w.ctl = chan of string; #titlebar->new(top, buts);
	tk->namechan(top, w.ctl, "wm_title");
...
#			w.wmctl(sys->sprint("!size . -1 %d %d", 0, 0));
			titlebar->sendctl(w.titlebar, "size");
		else if(p.buttons & 1){
			titlebar->sendctl(w.titlebar,sys->sprint("move %d %d", p.xy.x, p.xy.y));
