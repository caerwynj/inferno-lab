...
init(nil: ref Draw->Context, argv: list of string)
{
	...
	while((s := io.gets('\n')) != nil) {
		...
		if(voice >= len inst || voice < 0)
			continue;
		if(inst[voice] == nil){
			inst[voice] = Sig.open(hd argv);
			if(inst[voice] == nil)
				error("sequencer: bad inst");
		}
		case hd flds {
		"NoteOn" =>
			midi := int hd tl tl tl flds;
			e := fprint(inst[voice].ctl, "on %g 0.5", midi2pitch[midi%len midi2pitch]);
			if(e < 0)
				error("bad ctl write");
		"NoteOff" =>
			e := fprint(inst[voice].ctl, "off 0.5");
			if(e < 0)
				error("bad ctl write");
...
error(s: string)
{
	sys->fprint(sys->fildes(2), "sequencer: %s : %r\n", s);
	exit;
}