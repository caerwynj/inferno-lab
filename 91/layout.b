...
fonts := array[NumFnt] of {
	FntR*NumSize+Tiny => Fontinfo("/fonts/DejaVuSans.ttf", nil, 0, 8),
	FntR*NumSize+Small => ("/fonts/DejaVuSans.ttf", nil, 0, 10),
	FntR*NumSize+Normal => ("/fonts/DejaVuSans.ttf", nil, 0, 12),
	FntR*NumSize+Large => ("/fonts/DejaVuSans.ttf", nil, 0, 18),
	FntR*NumSize+Verylarge => ("/fonts/DejaVuSans.ttf", nil, 0, 24),
	
	FntI*NumSize+Tiny => ("/fonts/DejaVuSerif-Italic.ttf", nil, 0, 8),
	FntI*NumSize+Small => ("/fonts/DejaVuSerif-Italic.ttf", nil, 0, 10),
	FntI*NumSize+Normal => ("/fonts/DejaVuSerif-Italic.ttf", nil, 0, 12),
	FntI*NumSize+Large => ("/fonts/DejaVuSerif-Italic.ttf", nil, 0, 18),
	FntI*NumSize+Verylarge => ("/fonts/DejaVuSerif-Italic.ttf", nil, 0, 24),
	
	FntB*NumSize+Tiny => ("/fonts/DejaVuSans-Bold.ttf", nil, 0, 8),
	FntB*NumSize+Small => ("/fonts/DejaVuSans-Bold.ttf", nil, 0, 10),
	FntB*NumSize+Normal => ("/fonts/DejaVuSans-Bold.ttf", nil, 0, 12),
	FntB*NumSize+Large => ("/fonts/DejaVuSans-Bold.ttf", nil, 0, 18),
	FntB*NumSize+Verylarge => ("/fonts/DejaVuSans-Bold.ttf", nil, 0, 24),
	
	FntT*NumSize+Tiny => ("/fonts/DejaVuSansMono.ttf", nil, 0, 8),
	FntT*NumSize+Small => ("/fonts/DejaVuSansMono.ttf", nil, 0, 10),
	FntT*NumSize+Normal => ("/fonts/DejaVuSansMono.ttf", nil, 0, 12),
	FntT*NumSize+Large => ("/fonts/DejaVuSansMono.ttf", nil, 0, 18),
	FntT*NumSize+Verylarge => ("/fonts/DejaVuSansMono.ttf", nil, 0, 24)
};
...
init(cu: CharonUtils)
{
	...
	B = cu->B;
	display = G->display;
	ftfont = load Ftfont Ftfont->PATH;
...
getfont(num: int) : ref Font
{
	f := fonts[num].f;
	if(f == nil) {
		f = Font.open(display, fonts[num].name, fonts[num].pts);
		if(f == nil) {
			if(num == DefFnt)
...
drawline(f : ref Frame, layorigin : Point, l: ref Line, lay: ref Lay)
{
	...
	for(it := l.items; it != nil; it = it.next) {
		...
		Itext =>
			...
			fgi := colorimage(i.fg);
			stringx(im, Point(x, yy), fgi, zp, fnt.f, i.s);
			if(i.ul != ULnone) {
				if(i.ul == ULmid)
					yy += 2*i.ascent/3;
				else
...
drawimg(f: ref Frame, iorigin: Point, i: ref Item.Iimage)
{
	...
	if(ci.mims != nil) {
		...
		if(i.anchorid != 0)
			...
			col = DarkGrey;
		fgi := colorimage(col);
		stringx(im, Point(xx, yy), fgi, zp, fnt, i.altrep);
	}
}
...
{
	stringx(im, p, colorimage(Black), zp, fonts[DefFnt].f, s);
}
...
Control.draw(ctl: self ref Control, flush: int)
{
	...
	Cbutton =>
		...
		if(c.pic != nil) {
			...
			if(c.flags & CFactive)
				p = p.add(Point(1,1));
			stringx(win, p, colorimage(Black), zp, fonts[CtlFnt].f, c.label);
		}
		if(c.dorelief) {
			relief := ReliefRaised;
			if(c.flags & CFactive)
...
#			eclipr.max.x -= SCRFBREADTH;
		...
		for (n := 0; n < nlines; n++) {
			...
			for (; segs != nil; segs = tl segs) {
				...
				if (se > ss)
					stringx(win, q, txtcol, zp, fnt, txt);
				q.x += w;
			}
	...
	Cselect =>
		...
		for(i := c.first; i < len c.options && i < c.first+c.nvis; i++) {
			if(drawsel && c.options[i].selected) {
				...
				stringx(win, p, white, zp, fnt, c.options[i].display);
			}
			else {
				stringx(win, p, black, zp, fnt, c.options[i].display);
			}
			p.y += ctllinespace;
		}
	...
	Clistbox =>
		...
		for(i := c.first; i < len c.options && i < c.first+c.nvis; i++) {
			...
			if(c.options[i].selected) {
				...
				win.draw(r, navy, nil, zp);
				stringx(win, p, white, zp, fnt, txt);
			}
			else {
 				stringx(win, p, black, zp, fnt, txt);
			}
			p.y +=fnt.height;
		}
		if(c.vscr != nil) {
	...
	Clabel =>
		...
		stringx(win, p, colorimage(Black), zp, fonts[DefFnt].f, c.s);
	}
	if(flush) {
		if (ctl.popup != nil)
...
stringx(d : ref Image, p : Point, src : ref Image, sp: Point, f : ref Font, s : string)
{
#	d.text(p, src, sp, f, s);
	f.stringx(d, p, s, src);
}
