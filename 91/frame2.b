...
include "draw.m";
drawm: Draw;
Display, Point, Rect, Font, Image, Pointer : import drawm;
...
frdump(f : ref Frame)
{
	debug(sprint("nchars=%d\n", f.nchars));
	for (i := 0; i < f.nbox; i++) {
		debug(sprint("box %d : ", i));
		fb := f.box[i];
		if (fb.nrune >= 0)
...
xfrredraw(f : ref Frame, pt : Point)
{
	...
	for(nb = 0; nb < f.nbox; nb++) {
		...
		if(!f.noredraw && b.nrune >= 0)
			stringx(f.b, pt, f.font, b.ptr, f.cols[TEXT]);
		pt.x += b.wid;
	}
}
...
frdrawsel0(f : ref Frame, pt : Point, p0 : int, p1 : int, back : ref Image, text : ref Image)
{
	...
	for(nb=0; nb<f.nbox && p<p1; nb++){
		...
		if(b.nrune >= 0)
			stringx(f.b, pt, f.font, ptr[0:nr], text);
		pt.x += w;
		p += nr;
	}
...
frinittick(f : ref Frame)
{
	...
	ft = f.font;
	f.tick = nil;
	f.tick = display.newimage(((0, 0), (FRTICKW, ft.height)), display.image.chans, 0, Draw->White);
	if(f.tick == nil)
		return;
	f.tickback = display.newimage(f.tick.r, display.image.chans, 0, Draw->White);
	if(f.tickback == nil){
		f.tick = nil;
		return;
	}
