...
# ---m mmmm mmmc cccc cccc cccy yyyy
QID(m, c, y: int): int
{
...
#		usage();
	...
	(tchan, srv) = Styxserver.new(sys->fildes(0), Navigator.new(navops), big Qtopdir);
...
requestproc(req: chan of (ref Tmsg, ref Conn, ref Fid))
{
	...
	for(;;){
		...
		Write =>
			...
			Qctl =>
				if(c == nil)
					...
					if((s := c.ffs->config(string m.data)) == nil)
						srv.replydirect(ref Rmsg.Write(m.tag, len m.data));
					else
						srv.replydirect(ref Rmsg.Error(m.tag, s));
				}
		...
		Open =>
			...
			Qctl =>
				if(c.state == Inuse)
					srv.replydirect(ref Rmsg.Error(m.tag, "in use"));
			Qtopctl =>
				;
			* =>
