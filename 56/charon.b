...
{
	ffields := array[] of {"Text", "Password", "Checkbox", "Radio", "Submit", "Hidden", "Image",
		"Reset", "File", "Button", "Select", "Textarea"};
	if(ff.ftype != Build->Fhidden){
		s := "[" + ffields[ff.ftype] + sys->sprint(" %d.%d %s", ff.fieldid, ff.ctlid, ff.value) + "]";
		w.wwritebody(s);
	}
}
