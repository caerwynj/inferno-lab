...
mainmenu(display: ref Display, screen: ref Screen): ref Compound
{
	...
	textfont := Font.open(display, "/fonts/vera/Vera/Vera.14.font");
	textstyle := ref Style(
			textfont,			# titlefont
			textfont,			# textfont
			display.black,	# elemcolor
			display.color(Draw->Medgreen),	# edgecolor
			display.white,	# titlecolor	
			display.color(Draw->Medgreen),	# textcolor
			display.white			# highlightcolor
			);	
	...
	while(items != nil) {
		...
		ie = Element.icon(env, icon.r, icon, display.white);
		te.append(ie);
...
#	menu.clip(Rect(menu.r.min, menu.r.min.add((350, 230))));
	menu.clip(display.image.r);

	c := Compound.box(env, Point(20,10), Element.text(env, "Inferno DS", ((0,0),(0,0)), Prefab->ETitle), menu);
	c.draw();
	return c;
}
ZP := Point(0, 0);

drawbackground(display: ref Display): ref Image
{
	sys->print("drawbackground %d\n", display.image.r.max.x);
...
init(nil: ref Draw->Context, nil: list of string)
{
	...
	disp := display.image;
	screen := Screen.allocate(disp, display.black, 1);
