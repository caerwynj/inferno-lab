...
parseman(fd : ref Sys->FD, metrics : Metrics, ql : int, viewer : Viewman, setline : chan of list of (int, Text))
{
	iob := bufio->fopen(fd, Sys->OREAD);
	state := ref ParseState (metrics, ql, 0, nil, nil, 0, 0, metrics.indent, FONT_ROMAN, 0, 0, 1, nil, 0, 1, 0, "", nil, viewer, setline);
	while ((l := iob.gets('\n')) != nil) {
		if (l[len l -1] == '\n')
			l = l[0: len l - 1];
...
parseline(state : ref ParseState, t : string)
{
	if (t == nil) {
		if (state.verbatim) {
...
parsemacro(state : ref ParseState, t : string)
{
...
parsetext(state : ref ParseState, t : string) : list of Text
{
...
ParseState.addstring(state : self ref ParseState, s : string)
{
	t := Text(state.curfont, state.curattr, s, state.heading, state.link);
	state.addtext(t::nil);
}

ParseState.addtext(state : self ref ParseState, t : list of Text)
{
#dumptextlist(t);
...
ParseState.brk(state : self ref ParseState)
{
	if (state.curline != nil) {
...
ParseState.paragraph(state : self ref ParseState)
{
	state.brk();
	if (state.newpara == 0) {
		blank := Text(state.curfont, state.curattr, "", 0, "");
...
altattr(state : ref ParseState, f1, f2 : int, strs : list of string)
{
	index := 0;
	textl : list of Text;
