...
lookup(name, env: ref Sexp): ref Sexp
{
	if(null(env))
		...
		return lookup1(name, car(car(env)), cdr(car(env)), env);
}
...
apply(fun, args, env: ref Sexp): ref Sexp
{
	if(eq(car(fun), Procedure))
		return eval(car(cdr(cdr(fun))), bind(car(cdr(fun)), args, env));
	else
		return error("apply");
}

eval(n: ref Sexp, env: ref Sexp): ref Sexp
{
	if(n == nil)
		return nil;
	else if(atom(n)){
		...
		"cond" =>
			return evcon(cdr(n), env);
		"lambda" =>
			return cons(Procedure, cdr(n));
		"define" =>
			Env = bind(ref Sexp.List(car(car(cdr(n))) :: nil), 
				ref Sexp.List(ref Sexp.List(Procedure :: cdr(car(cdr(n))) :: car(cdr(cdr(n))) :: nil) :: nil), 
				Env);
		* =>
			return apply(value(car(n), env), evlis(cdr(n), env), env);
		}
		}
	}else{
		nn := cdr(n);
