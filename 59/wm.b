...
#		sys->fprint(sys->fildes(2), "wmsize %s\n", r2s(wmsz));
		wmclient->win.wmctl(sys->sprint("!reshape . -1 %s", r2s(wmsz)));
...
reshaped(win: ref Wmclient->Window, newr: Rect)
{
	oldr := screen.image.r;
#	newr := win.image.r;
	mx := Fix;
...
{
	screen = Screen.allocate(img, img.display.colormix(Draw->Paleyellow, Draw->Grey), 0);
